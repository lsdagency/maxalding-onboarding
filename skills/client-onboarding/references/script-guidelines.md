...
## Hook sets need one genuinely creative option (Feedback 2026-09-18, Edge Fit Stanmore)
When a concept carries more than one hook (a hook test), the set must not be five callouts in five wordings. Span real types, keep them gain led (see the negative to positive arc in writing-rules.md), and make at least ONE option properly creative for this audience on this platform: something camera native or local that a generic coach could not say. Two that worked: a demonstrative opener that shows the room before describing it ("Have a look behind me. This is a normal weekday morning in here", filmed with real members training in the background), and a local price comparison ("Ninety dollars. That's about a dinner out in Newtown. Or it's three personal training sessions with us"). Any creative hook that needs something in frame gets its own line in FILMING DIRECTION.
