...
- Brand memory: real voice patterns, recurring phrases, natural content formats, visual grammar.
- **The client-only vocabulary list is MANDATORY.** While the transcripts and captions are open, write down the specialist terms the client uses WITH THE AGENCY (in the onboarding document, on a call) that appear NOWHERE in their own audience-facing content. Grep the transcripts and captions for each candidate; a zero count puts the term on the list. That list is banned from every hook, headline, static overlay, lead form intro and VSL opener for the whole build. Such a term may appear only where the asset explains it in the same breath, for example a Benefit concept whose job is answering "what am I buying". Record the list in the Brand memory next to the "How [name] actually talks" block. This is the third recurrence of the same failure (Xcelerate 2026-08-11 two registers, Max Potential 2026-08-26 HYROX, Oliver Bird 2026-09-07 force plates, where a term with zero occurrences across every transcribed reel had reached 9 of 35 posts, a headline, two video hooks and the lead form intro), so build the list mechanically rather than trusting judgement. The "words to use" list should prefer words the client demonstrably uses.
