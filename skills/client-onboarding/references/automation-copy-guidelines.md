# CRM Automation copy guidelines

Document 4, built with build/crm_automation.py. Apply email-sequence principles before writing any email. Same typography and header, no date, logo at top.
...
The four workflows below belong to the lead-gen side of the funnel, which the agency builds in its own GoHighLevel for EVERY client. That is a given, not a question: do not ask which CRM the copy is going into, and do not read a client document that says they have no CRM as a missing input. The only platform question worth asking is whether the client runs a SECOND system of their own, covered below. The thing being booked, confirmed and chased in them is a QUICK CALL with the coach, never the end service itself. Do not write the Booked Call Confirmation as if the lead has booked the session, consultation or class: no studio directions, no what-to-wear notes, no arrival instructions. Confirm a call, set expectations for a call (a relaxed chat, questions answered, decide if the front-end offer is the right next step), and use the meeting link.
...
For each workflow: the WORKFLOW NAME in caps, a TRIGGER, then each message in order.
- Emails: a number and bold subject, Delay, an italic direction note, then Subject, Body and a single CTA, with body and CTA copy in grey.
- SMS: a number, Delay, an italic note, then the SMS copy in grey.
...
Each sequence has its own intent: Lead Follow Up pushes to book; Booked Call Confirmation confirms, sets expectations and reminds before the call to maximise show rate; No Show Rebook recovers fast and frictionless; Long Term Nurture delivers value and authority with a soft CTA.
