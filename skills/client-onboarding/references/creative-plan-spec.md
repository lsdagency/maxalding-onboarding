...
Video HOOK rules: three hooks per row, at least one statement, never "you" plus a negative attribute, specific to this client. Static HOOK rules: three hook variations, only the hook changes across the three graphic variations. Audience Addresser is the exception: three hooks calling the viewer out by age range, job or role, and daily-life situation; questions are encouraged.
...
- Emoji and length are fluid and intentional, never a pattern. Lean into emoji: aim for around a third of the posts to carry one, varied and never one-per-row, with some left bare. Prefer premium, on-brand emoji (sparkle, white heart, warm heart, leaf, location pin, house, sun) over hype or loud ones (no fire, 100, rocket, heart-eyes), subject to brand tone. Do not pad copy to reach the character limit: if shorter reads stronger, keep it shorter.
