...
Required: at least one of the Brand Campaign Document or the Creative Strategy Brief, AND the client's Instagram handle or profile URL. If neither document is present, stop and say: "To run the onboarding I need at least one of the following: the Brand Campaign Document or the Creative Strategy Brief. Please upload one or both and I will get started." If the Instagram account is missing, ask for it in the same message; the only waiver is the user explicitly stating the client has no Instagram. Do not work from memory or conversation context. If only one is present, proceed but flag what is missing and recommend a human creative review. If the funnel goal is not stated anywhere, ask before proceeding.

## The four stages
- Stage 1, Read everything: delegate to the intake-research subagent. It reads the feedback log and every document and transcript in full, and runs the compulsory Instagram scan (references/instagram-research.md): captions and metadata, the five most recent videos downloaded and transcribed, visual style and language read.
