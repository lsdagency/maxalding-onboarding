...
2. Onboarding: full name, business name, contact, address, website, socials, asset folder link, current income, short and long term goals, current lead volume and source, CRM, key dates. The CRM field records any system the CLIENT already runs (booking, membership, payments), for platform-splitting only. It never decides whether the CRM Automation deliverable is written or where it goes: the agency builds every client's lead follow-up emails and SMS in its own GoHighLevel, so never ask at the gate which CRM the automations are for, and never treat a blank or "NA" as a gap (Feedback 2026-09-18, Edge Fit Stanmore).
...
## Define the campaign spine (every time)
Produce a single Core Message the whole campaign works back to, a short Tagline, and one supporting line. Derive them from the client's authentic positioning and unique mechanism. These thread through every later asset and appear in the Brand Positioning section of the Creative Plan.
...
Plain human English. Never fabricate. Quantified outcomes such as a specific weight figure may only be used if explicitly present in the client documents. Keep all writable state in the workspace folder, never in the package.
